  ...
  return (
    <main className="space-y-6">
      ...
      <section
        ...
        className="mx-auto flex max-w-xs flex-col items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-6 text-center text-zinc-900 print:border-0"
      >
        <p className="text-xl font-semibold leading-tight">{nombre}</p>
        <p className="text-sm text-zinc-600">Escanea para ver el menú</p>
      ...
      <div className="flex flex-wrap justify-center gap-2 print:hidden">
        ...
        <BotonImprimir />
        <a href={qr.svg} download={`qr-${slug}.svg`} className="rounded-full border border-zinc-300 px-4 py-2 text-sm font-medium dark:border-zinc-700">
          Descargar SVG
        </a>
        <a href={qr.png} download={`qr-${slug}.png`} className="rounded-full border border-zinc-300 px-4 py-2 text-sm font-medium dark:border-zinc-700">
          Descargar PNG
        </a>
      </div>
