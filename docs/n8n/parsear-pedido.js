// GENERADO por scripts/generar-parser-n8n.mjs desde src/lib/pedido-agente.ts. No lo edites a mano.
...
function codigoProducto(id) {
    return id.slice(0, 8).toLowerCase();
}
function construirBloque(datos) {
...
const fallo = (error, detalle) => ({ ok: false, error, detalle });
...
function parsearPedido(texto) {
    ...
    if (indices.length > 1)
        return fallo("multiples_bloques");
    const m = /^\[\[PEDIDO v(\d+)\|([^\[\]]*)\]\]$/.exec(lineas[indices[0]]);
    if (!m)
        return fallo("bloque_malformado");
    if (m[1] !== String(VERSION_FORMATO))
        return fallo("version_no_soportada", m[1]);
    const campos = {};
    for (const par of m[2].split("|")) {
        const corte = par.indexOf("=");
        if (corte < 1)
            return fallo("bloque_malformado", par);
        const clave = par.slice(0, corte);
        if (!CAMPOS.includes(clave))
            return fallo("campo_desconocido", clave);
        if (clave in campos)
            return fallo("campo_repetido", clave);
        campos[clave] = par.slice(corte + 1);
    }
    for (const c of CAMPOS)
        if (!(c in campos))
            return fallo("campo_faltante", c);
    ...
    if (partes.length < 1 || partes.length > 50)
        return fallo("items_invalidos");
    const porCodigo = new Map();
    for (const p of partes) {
        const it = /^([0-9a-f]{8})x([1-9][0-9]{0,2})$/.exec(p);
        if (!it)
            return fallo("items_invalidos", p);
        const suma = (porCodigo.get(it[1]) ?? 0) + Number(it[2]);
        if (suma > 999)
            return fallo("items_invalidos", it[1]);
        porCodigo.set(it[1], suma);
    }
    if (!/^\d{1,8}\.\d{2}$/.test(campos.total))
        return fallo("total_invalido");
    if (!/^\d{1,7}(\.\d{1,4})?$/.test(campos.tasa))
        return fallo("tasa_invalida");
    if (campos.entrega !== "retiro" && campos.entrega !== "domicilio")
        return fallo("entrega_invalida");
    // Texto libre: primera línea con cada prefijo, siempre por encima del bloque.
    const previas = lineas.slice(0, indices[0]);
    const buscar = (prefijo, max) => {
